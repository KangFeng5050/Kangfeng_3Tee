...
__inline static HI_U32 GetSysTimeBySec(void)
{
    ...
    #ifdef __KERNEL__
        osal_timeval_t stTime;
        osal_gettimeofday(&stTime);
    #else
...
#define hi_usleep(usec) usleep(usec)
#else
...
__inline static HI_S32 snprintf_s (HI_CHAR* strDest, HI_U32 destMax, HI_U32 count, const HI_CHAR* format, ...)
{
    ...
    if (destMax > count)
    {
		...
		s32Ret = ((s32Ret < 0) || ((HI_U32)s32Ret < count)) ? s32Ret : count;
    }
    else /* destMax <= count */
    {
        s32Ret = vsnprintf(strDest, destMax, format, args);
		s32Ret = ((s32Ret < 0) || ((HI_U32)s32Ret < destMax-1)) ? s32Ret : destMax-1;
	}
