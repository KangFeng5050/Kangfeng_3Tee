...
HI_VOID	HI3531D_COMM_VDEC_Sysconf(VB_CONF_S *pstVbConf, SIZE_S *pstSize)
{
    ...
    pstVbConf->astCommPool[0].u32BlkSize = 1920 * 1080 * 2;
...
HI_VOID	HI3531D_COMM_VDEC_ChnAttr(HI_S32 s32ChnNum, 
...
HI_VOID HI3531D_COMM_VDEC_ThreadParam(HI_S32 s32ChnNum, VdecThreadParam *pstVdecSend, 
...
HI_VOID HI3531D_MST_GetDefVoAttr(VO_DEV VoDev, 
...
HI_VOID HI3531D_MST_StartPIPVideoLayer(VO_DEV VoDev, VO_VIDEO_LAYER_ATTR_S *pstLayerAttr,
...
HI_VOID * HI3531D_COMM_VDEC_SendStream(HI_VOID *pArgs)
{
   ...
   // {
        ...
        if ( (pstVdecThreadParam->s32StreamMode==VIDEO_MODE_FRAME) && (pstVdecThreadParam->enType == PT_MP4VIDEO) )
        {
            ...
            {
                printf("HI3531D_TEST: chn %d can not find start code! s32ReadLen %d, s32UsedBytes %d. \n", 
	...
	fwrite(pu8Buf,1,s32ReadLen,My_fd);
   ...
   // }
    ...
    //printf("HI3531D_TEST:send steam thread %d return ...\n", pstVdecThreadParam->s32ChnId);
    fflush(stdout);
    if (pu8Buf != HI_NULL)
    {
        free(pu8Buf);
...
HI_VOID * HI3531D_COMM_VDEC_GetChnLuma(HI_VOID *pArgs)
{
    ...
    if(fpLuma == NULL)
    {
        printf("HI3531D_TEST:can't open file %s in get luma thread:%d\n", pstVdecThreadParam->cFileName, pstVdecThreadParam->s32ChnId);
        return (HI_VOID *)(HI_FAILURE);
    }	
    ...
    while(1)
    {
        switch(pstVdecThreadParam->eCtrlSinal)
        {
            ...
            default:
                printf("HI3531D_TEST:unknow command in get luma thread %d\n", pstVdecThreadParam->s32ChnId);
                bRunFlag=HI_FALSE;
                break;
        }
        usleep(40000);
    ...
    printf("HI3531D_TEST:get LumaPix thread %d return ...\n", pstVdecThreadParam->s32ChnId);
    fclose(fpLuma);	
...
HI_VOID HI3531D_COMM_VDEC_CmdCtrl(HI_S32 s32ChnNum,VdecThreadParam *pstVdecSend)
{
    ...
    while(1)    
    {
        printf("\nHI3531D_TEST:press 'e' to exit; 'p' to pause/resume; 'q' to query!;'s' to step!;'a' to add!;'d' to sub!;\n"); 
...
HI_VOID HI3531D_COMM_VDEC_StartSendStream(HI_S32 s32ChnNum, VdecThreadParam *pstVdecSend, pthread_t *pVdecThread)
{
    ...
    for(i=0; i<s32ChnNum; i++)
    {
        pthread_create(&pVdecThread[i], 0, HI3531D_COMM_VDEC_SendStream, (HI_VOID *)&pstVdecSend[i]);
    }
}
...
HI_VOID HI3531D_COMM_VDEC_StartGetLuma(HI_S32 s32ChnNum, VdecThreadParam *pstVdecSend, pthread_t *pVdecThread)
{
    ...
    for(i=0; i<s32ChnNum; i++)
    {
		...
		{
			pthread_create(&pVdecThread[i+VDEC_MAX_CHN_NUM], 0, HI3531D_COMM_VDEC_GetChnLuma, (HI_VOID *)&pstVdecSend[i]);	
		}
    }
}
