...
#if 0
HI_S32 HI3531A_COMM_VO_StartChn(VO_LAYER VoLayer, SAMPLE_VO_MODE_E enMode) 
{
    HI_S32 i;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32WndNum = 0;
...
HI_S32 HI3531D_COMM_VO_StopChn(VO_LAYER VoLayer, SAMPLE_VO_MODE_E enMode)
{
    HI_S32 i;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32WndNum = 0;
...
HI_S32 HI3531D_COMM_Vpss_BindVpss(VPSS_GRP VpssDestGrp,VO_CHN VoChn,VPSS_GRP VpssSrcGrp,VPSS_CHN VpssChn)
{
    HI_S32 s32Ret = HI_SUCCESS;
    MPP_CHN_S stSrcChn;
    MPP_CHN_S stDestChn;
...
HI_S32 HI3531D_COMM_VO_UnBindVi(VO_LAYER VoLayer, VO_CHN VoChn)
{
    MPP_CHN_S stDestChn;
...
static HI_VOID HI3531D_COMM_VO_HdmiConvertSync(VO_INTF_SYNC_E enIntfSync,
...
HI_S32 HI3531D_COMM_VO_HdmiStart(VO_INTF_SYNC_E enIntfSync)
{
    ...
    HI3531D_COMM_VO_HdmiConvertSync(enIntfSync, &enVideoFmt);
    ...
    ******************************************/
    ...
    else
    {
        if (FD_ISSET(s32VencFd, &read_fds))
        {
            ...
            s32Ret = HI3531D_COMM_VO_SaveSnap(&stStream);
            if (HI_SUCCESS != s32Ret)
            {
                SAMPLE_PRT("HI_MPI_VENC_GetStream failed with %#x!\n", s32Ret);
                free(stStream.pstPack);
...
HI_S32 HI3531D_COMM_start_VO(VPSS_GRP VpssGrp,VPSS_CHN VpssChn)
{
	...
	s32Ret = HI3531D_COMM_VO_StartDev(VoDev, &stVoPubAttr_hd1);
	if (HI_SUCCESS != s32Ret)
	{
	...
	s32Ret = HI3531D_COMM_VO_GetWH(stVoPubAttr_hd1.enIntfSync, \
	...
	//s32Ret = HI3531D_COMM_VO_StartLayer(VoLayer, &stLayerAttr,HI_TRUE);
	//if (HI_SUCCESS != s32Ret)
	//{
	...
	//s32Ret = HI3531D_COMM_VO_StartChn(VoLayer, VoChn);
	if (HI_SUCCESS != s32Ret)
	{
	...
	{
		if (HI_SUCCESS != HI3531D_COMM_VO_HdmiStart(stVoPubAttr_hd1.enIntfSync))
		{
			SAMPLE_PRT("Start SAMPLE_COMM_VO_HdmiStart failed!\n");
			
		}
	}
	s32Ret = HI3531D_COMM_VO_BindVpss(VoDev,VoChn,VpssGrp,VpssChn);
	if (HI_SUCCESS != s32Ret)
	{
